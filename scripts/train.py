...
from moses.distribute_utils import init_distributed_mode, dist
...
def main(model, config):
    ...
    if config.config_save is not None and rank == 0:
    ...
    if config.vocab_save is not None and rank == 0:
    ...
    trainer.fit(model, train_data, val_data, rank)
    if rank == 0:
        model = model.to('cpu')
        torch.save(model.state_dict(), config.model_save)
